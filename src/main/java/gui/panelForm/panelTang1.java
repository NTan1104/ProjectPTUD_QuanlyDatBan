...
public class panelTang1 extends JPanel {
    ...
    private void showChuyenBanDialog(String tenBan, String maKH, String status) {
        if (!status.equals("ĐANG SỬ DỤNG") && !status.equals("ĐÃ ĐẶT")) {
            JOptionPane.showMessageDialog(this, "Bàn này chưa được sử dụng, không thể chuyển!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        String maHD = hoaDon_DAO.getMaHDByMaKHAndStatus(maKH, "Chưa thanh toán");
        if (maHD == null) {
            JOptionPane.showMessageDialog(this, "Không có hóa đơn nào đang được tạo cho bàn này!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        tablePanel.setLayout(new GridLayout(0, 5, 10, 10)); // 5 columns, dynamic rows
        ...
        btnXacNhan.setBackground(new Color(0, 120, 215)); // Blue similar to the image
        ...
        btnHuy.setBackground(new Color(255, 99, 71)); // Red similar to the image
        ...
        btnXacNhan.addActionListener(e -> {
            ...
            // Update MaBan in ChiTietPhieuDatBan (already handled)
            ...
            Ban_DAO.capNhatTrangThaiBan(tenBan, "CÒN TRỐNG");
            Ban_DAO.capNhatTrangThaiBan(newTenBan, "ĐANG SỬ DỤNG");

            // Refresh the UI
            refreshBanList();
            displayBanDetails(newTenBan, "ĐANG SỬ DỤNG", maKH, tenKH, sdt, gioiTinh, soLuong);
